"""empty message

Revision ID: ee8ecd4042a5
Revises: 
Create Date: 2020-10-26 16:37:06.403988

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = 'ee8ecd4042a5'
down_revision = None
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.create_table('cupons',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('cupom', sa.String(length=15), nullable=False),
    sa.Column('dataCriacao', sa.DateTime(), nullable=True),
    sa.Column('dataExpiracao', sa.DateTime(), nullable=False),
    sa.Column('habilitado', sa.Boolean(), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('cupom')
    )
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_table('cupons')
    # ### end Alembic commands ###
